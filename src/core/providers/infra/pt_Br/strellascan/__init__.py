...
class StrellaScanProvider(Base):
    name = 'Strella Scans'
    lang = 'pt_Br'
    ...
    def getChapters(self, id: str) -> List[Chapter]:
        ...
        soup = BeautifulSoup(response.content, 'html.parser')
        title = soup.select_one('header h1')
        response = Http.get(f'https://www.strellascan.xyz/feeds/posts/default/-/{title.get_text(strip=True)}?alt=json&start-index=1&max-results=150').json()
        list = []
